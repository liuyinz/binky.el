  ...
  (when (and binky-mode binky-margin-mode)
    ...
    (mapc (lambda (x)
            (let ((pos (marker-position (cdr x))))
              (with-current-buffer (marker-buffer (cdr x))
...
(define-minor-mode binky-margin-mode
  ...
  (binky--margin-setup (not binky-margin-mode))
  (binky-margin-update))
